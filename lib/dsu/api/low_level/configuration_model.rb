    ...
    module LowLevel
      ...
      class ConfigurationModel < JsonFile
        ...
        DEFAULT_CONFIGURATION = {
          version: VERSION,
          # The default editor to use when editing entry_model groups if the EDITOR
          ...
          # Models::ColorThemeModel::DEFAULT_THEME_NAME or the name of a custom
        ...
        # Temporarily sets the configuration_model to the given config_hash.
        # To reset the configuration_model to its original state, call #reload
        ...
        # Restores the configuration_model to its original state from disk.
        def reload
          file_hash = if exist?
            read do |config_hash|
              hydrated_hash = Services::ConfigurationModel::HydratorService.new(config_hash: config_hash).call
        ...
        def ==(other)
          return false unless other.is_a?(ConfigurationModel)
        ...
        def validate_theme_file
          ...
          i18n_key = 'configuration_model.errors.theme_file_missing'
