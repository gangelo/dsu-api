...
RSpec.describe Dsu::Api::LowLevel::EntryGroupModel do
  subject(:entry_group) { build(:entry_group, time: time, entries: entries) }

  before do
    build(:configuration, config_hash: Dsu::Api::LowLevel::ConfigurationModel::DEFAULT_CONFIGURATION)
  end
  ...
  describe 'validation' do
    it 'validates #version attribute with the VersionValidator' do
      expect(described_class).to validate_with_validator(Dsu::Api::LowLevel::Validators::VersionValidator)
    end

    it 'validates #entries attribute with the EntriesValidator' do
      expect(described_class).to validate_with_validator(Dsu::Api::LowLevel::Validators::EntriesValidator)
    end

    it 'validates #time with TimeValidator' do
      expect(described_class).to validate_with_validator(Dsu::Api::LowLevel::Validators::TimeValidator)
    end
    ...
    describe '.delete' do
      context 'when an entry group file exists for :time' do
        ...
        it 'deletes the file' do
          ...
          entry_path = Dsu::Api::LowLevel::Modules::Fileable.entries_path(time: time)
