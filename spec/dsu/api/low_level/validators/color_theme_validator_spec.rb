...
RSpec.describe Dsu::Api::LowLevel::Validators::ColorThemeValidator do
  subject(:color_theme_validator) do
    Dsu::Api::LowLevel::ColorThemeModel.new(theme_name: theme_name, theme_hash: theme_hash)
  end
  ...
  let(:theme_name) { 'test' }
  let(:theme_hash) { Dsu::Api::LowLevel::ColorThemeModel::DEFAULT_THEME.dup }
  ...
  context 'when there are extra color theme key/value pairs' do
    let(:theme_hash) do
      Dsu::Api::LowLevel::ColorThemeModel::DEFAULT_THEME.dup.tap do |hash|
        hash[:extra] = 'extra'
      end
    end
