...
RSpec.describe Dsu::Api::LowLevel::Services::MigrationService do
  ...
  shared_examples 'the migration is successful' do
    it 'backs up the old config file' do
      expect(File.exist?(File.join(backup_folder, Dsu::Api::LowLevel::Modules::Fileable.config_file_name))).to be true
    end

    it 'backs up the old entry files' do
      entries_folder = File.basename(Dsu::Api::LowLevel::Modules::Fileable.entries_folder)
      expect(File.exist?(File.join(backup_folder, entries_folder, Dsu::Api::LowLevel::Modules::Fileable.entries_file_name(time: time)))).to be true
    end

    it 'backs up the old theme files' do
      themes_folder = File.basename(Dsu::Api::LowLevel::Modules::Fileable.themes_folder)
      expect(File.exist?(File.join(backup_folder, themes_folder, Dsu::Api::LowLevel::Modules::Fileable.theme_file_name(theme_name: theme_name)))).to be true
    end

    it 'creates the new config file' do
      expect(File.exist?(Dsu::Api::LowLevel::Modules::Fileable.config_path)).to be true
    end

    it 'copies the new theme files' do
      theme_names = %w[cherry default lemon matrix whiteout]
      themes_exist = theme_names.all? { |theme_name| File.exist?(Dsu::Api::LowLevel::Modules::Fileable.themes_path(theme_name: theme_name)) }
      expect(themes_exist).to be true
    end

    it 'creates an initial entry group' do
      expect(File.exist?(Dsu::Api::LowLevel::Modules::Fileable.entries_path(time: time))).to be true
    end
  end
  ...
  describe 'class methods' do
    describe '.run_migrations?' do
      ...
      context 'when the migration version less than the current version' do
        before do
          create(:migration_version, version: Dsu::Api::LowLevel::Migration::VERSION - 1)
        end
  ...
  context 'when migrations should be run' do
    ...
    before do
      FileUtils.touch(Dsu::Api::LowLevel::Modules::Fileable.config_path)
      FileUtils.touch(Dsu::Api::LowLevel::Modules::Fileable.entries_path(time: time))
      FileUtils.touch(Dsu::Api::LowLevel::Modules::Fileable.themes_path(theme_name: theme_name))
    end

    let(:time) { Time.now.in_time_zone }
    let(:theme_name) { 'old_theme' }
    let(:backup_folder) { Dsu::Api::LowLevel::Modules::Fileable.backup_folder(version: 0) }
